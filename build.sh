...
if [ "$BACKEND" = "dawn" ]; then
  cmake -B "$BUILD_DIR" -DWEBGPU_BACKEND=$BACKEND -DDEV_MODE=$DEV_MODE -DCMAKE_BUILD_TYPE=$BUILD_TYPE "$SCRIPT_DIR"
elif [ "$BACKEND" = "wgpu" ]; then
  # echo "cmake -B \"$BUILD_DIR\" -DWEBGPU_BACKEND=$BACKEND -DDEV_MODE=$DEV_MODE -DCMAKE_BUILD_TYPE=$BUILD_TYPE \"$SCRIPT_DIR\""
